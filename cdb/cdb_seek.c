...
uint32 *dlen;
{
  ...
  if (lseek(fd,(off_t) pos,SEEK_SET) == -1) {
    printf("cdb_seek error 1\n");
    return -1;
  }

  if (cdb_bread(fd,packbuf,8) == -1) { 
    printf("cdb_seek error 2\n");
    return -1;
  }
  ...
  for (loop = 0;loop < lenhash;++loop) {
    if (lseek(fd,(off_t) (pos + 8 * h2),SEEK_SET) == -1) {
      printf("cdb_seek error 3\n");
      return -1;
    }
    if (cdb_bread(fd,packbuf,8) == -1) {
      printf("cdb_seek error 4\n");
      return -1;
    }
    ...
    if (cdb_unpack(packbuf) == h) {
      if (lseek(fd,(off_t) poskd,SEEK_SET) == -1) {
        printf("cdb_seek error 5\n");
        return -1;
      }
      if (cdb_bread(fd,packbuf,8) == -1) { 
        printf("cdb_seek error 6\n");
        return -1;
      }
      if (cdb_unpack(packbuf) == len)
	switch(match(fd,key,len)) {
	  case -1:
            printf("cdb_seek error 7\n");
	    return -1;
	  case 1:
